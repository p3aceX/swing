...
class _MatchDetailScreenState extends ConsumerState<MatchDetailScreen>
  ...
  void _ensureLiveSync() {
    ...
    _overlayStreamSub = ref
        ...
        .watchLiveOverlay(widget.matchId)
        .listen(
      (payload) {
  ...
  void _ensureFallbackRefresh() {
    ...
    _fallbackRefreshTimer = Timer.periodic(const Duration(seconds: 3), (_) {
      if (!_liveSyncEnabled || !mounted) {
        return;
      }

      final lastSignalAt = _lastOverlaySignalAt;
      final streamIsStale = lastSignalAt == null ||
          DateTime.now().difference(lastSignalAt) > const Duration(seconds: 4);

      if (streamIsStale) {
        _refreshLiveProviders(force: true);
        if (_overlayStreamSub == null && !_overlayConnecting) {
          _ensureLiveSync();
        }
      }
  ...
  void _refreshLiveProviders({bool force = false}) {
    ...
    ref.invalidate(matchCenterProvider(widget.matchId));
    ref.invalidate(matchCommentaryProvider(widget.matchId));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final centerAsync = widget.matchId.trim().isNotEmpty
        ? ref.watch(matchCenterProvider(widget.matchId))
        : null;

    final center = centerAsync?.valueOrNull ?? fallback;
    final isLoading = centerAsync?.isLoading == true;

    // Start/stop auto-refresh based on match lifecycle
    _syncPolling(center?.lifecycle);
...
class _HeroHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colA = _teamColor(center.teamAName);
    final colB = _teamColor(center.teamBName);
    final live = center.liveState;
    final hasYoutube = _has(center.youtubeUrl);

    // Batting-first team always on left
    final firstBattingTeam = center.innings
    ...
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            rankTheme.deep,
            Color.lerp(rankTheme.deep, rankTheme.primary, 0.08)!,
            rankTheme.deep,
          ],
        ),
      ),
    ...
    return FadeTransition(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: const Color(0xFFFF3B30).withValues(alpha: 0.30), width: 1),
        ),
        child: const Row(
          mainAxisSize: MainAxisSize.min,
          children: [
...
class _StatusPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = switch (center.lifecycle) {
      MatchLifecycle.upcoming => ('Upcoming', const Color(0xFFD7A94B)),
      MatchLifecycle.past => ('Completed', Colors.white38),
      MatchLifecycle.live => ('Live', const Color(0xFFFF6B64)),
    };
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
      decoration: BoxDecoration(
...
class _BallDot extends StatelessWidget {
  ...
  static Color _bg(String l) => switch (l) {
        'W' => const Color(0xFFFF3B30),
        '4' => const Color(0xFF2E7D32),
        '6' => const Color(0xFF1565C0),
        'WD' || 'NB' => const Color(0xFFD7A94B),
        _ => Colors.white12,
      };

  static Color _fg(String l) => switch (l) {
        'WD' || 'NB' => Colors.black87,
        _ => Colors.white,
      };
  ...
  Widget build(BuildContext context) => Container(
        width: 26,
        height: 26,
        decoration: BoxDecoration(color: _bg(label), shape: BoxShape.circle),
        child: Center(
          child: Text(
            label,
            style: TextStyle(
                  ...
                  child: Row(children: [
                    Expanded(
                      ...
                      child: Row(children: [
                        ...
                        if (b.isStriker) ...[
                          const SizedBox(width: 4),
                          const _Dot(color: Color(0xFF4CAF50)),
                        ],
                      ]),
                    ),
...
class _ScoreSummaryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final leftScore = leftIsA ? center.teamAScore : center.teamBScore;
    final rightScore = leftIsA ? center.teamBScore : center.teamAScore;
    final leftColor = _teamColor(leftName);
    final rightColor = _teamColor(rightName);
...
class _CompletedOverviewCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final winnerColor = summary.winnerTeam == center.teamAName
        ? _teamColor(center.teamAName)
        : summary.winnerTeam == center.teamBName
            ? _teamColor(center.teamBName)
            : context.gold;
                      ...
                      Expanded(
                        ...
                        child: InkWell(
                          onTap: b.playerId != null
                              ? () => context.push('/player/${b.playerId}')
                              : null,
                          child: Row(
                            children: [
                              Flexible(
                ...
                Padding(
                  ...
                  child: Text(
                    ...
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                          color:
                              b.isOut ? context.fgSub : const Color(0xFF4CAF50),
                          fontStyle:
        ...
        Expanded(
          child: TabBarView(
            ...
            children: squads.map((squad) {
              final color = _teamColor(squad.teamName);
              return ListView(
                      ...
                      children: squad.players.asMap().entries.map((e) {
                        ...
                        return InkWell(
                          onTap: p.playerId != null
                              ? () => context.push('/player/${p.playerId}')
                              : null,
                          child: Column(
                            children: [
                              Padding(
...
class _ManhattanChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const wicketColor = Color(0xFFEF5350);
      ...
      child: Column(
        children: [
          ...
          const Wrap(
            ...
            children: [
              _WwLegend(color: Color(0xFF9E9E9E), label: 'Dot'),
              _WwLegend(color: Color(0xFF42A5F5), label: '1–3'),
              _WwLegend(color: Color(0xFF66BB6A), label: '4'),
              _WwLegend(color: Color(0xFFFFCA28), label: '6'),
              _WwLegend(color: Color(0xFFEF5350), label: 'Wicket'),
            ],
          ),
        ],
      ),
...
class _WagonWheelPainter extends CustomPainter {
  ...
  Color _colorForBall(WagonWheelBall b) {
    if (b.isWicket) return const Color(0xFFEF5350);
    if (b.runs == 6) return const Color(0xFFFFCA28);
    if (b.runs == 4) return const Color(0xFF66BB6A);
    if (b.runs >= 1) return const Color(0xFF42A5F5);
...
class _MvpTabState extends State<_MvpTab> {
  ...
  Widget build(BuildContext context) {
            ...
            Container(
              ...
              child: Column(
                children: leaderboard.asMap().entries.map((item) {
                  ...
                  final teamColor = _teamColor(entry.teamName);
    ...
    final colA = _teamColor(effectiveCenter.teamAName);
    final colB = _teamColor(effectiveCenter.teamBName);
...
class _CommentaryInline extends StatelessWidget {
  ...
  Color _color(BuildContext context) {
    if (entry.isWicket) return const Color(0xFFE53935);
    if (entry.outcome == 'SIX') return const Color(0xFF7B1FA2);
    if (entry.outcome == 'FOUR') return const Color(0xFF1565C0);
...
class _CommentaryTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(matchCommentaryProvider(matchId));
    return async.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      ...
      data: (entries) {
        if (entries.isEmpty) {
          return const _EmptyState(
              'Commentary will appear ball by ball once the match begins.',
              Icons.sports_cricket_rounded);
        }

        // Group by inningsNumber + overNumber
        final byOver = <({int inn, int over}), List<MatchCommentaryEntry>>{};
        for (final e in entries) {
          final key = (inn: e.inningsNumber, over: e.overNumber);
          byOver.putIfAbsent(key, () => []).add(e);
        }
        // Overs newest-first
        final sortedKeys = byOver.keys.toList()
          ..sort((a, b) {
            if (a.inn != b.inn) return b.inn.compareTo(a.inn);
...
class _OverGroupState extends State<_OverGroup> {
  ...
  Color _ballColor(MatchCommentaryEntry e) {
    if (e.isWicket) return const Color(0xFFE53935);
    if (e.outcome == 'SIX') return const Color(0xFF7B1FA2);
    if (e.outcome == 'FOUR') return const Color(0xFF1A7ACF);
...
class _BallRow extends StatelessWidget {
  ...
  Color _color(BuildContext context) {
    if (entry.isWicket) return const Color(0xFFE53935);
    if (entry.outcome == 'SIX') return const Color(0xFF9C27B0);
    if (entry.outcome == 'FOUR') return const Color(0xFF1A7ACF);
    return context.fgSub;
  }
...
String _teamAbbr(String name) {
  ...
  return trimmed.substring(0, trimmed.length.clamp(0, 4)).toUpperCase();
}
