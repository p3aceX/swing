...
final _detailProvider =
    FutureProvider.autoDispose.family<TournamentDetail, String>(
  (ref, slug) =>
      ref.watch(tournamentsRepositoryProvider).fetchTournamentDetail(slug),
);

final _matchesProvider =
    FutureProvider.autoDispose.family<List<TournamentMatch>, String>(
  (ref, slug) =>
      ref.watch(tournamentsRepositoryProvider).fetchTournamentMatches(slug),
);

final _standingsProvider =
    FutureProvider.autoDispose.family<List<TournamentStanding>, String>(
  (ref, slug) =>
      ref.watch(tournamentsRepositoryProvider).fetchTournamentStandings(slug),
);

final _leaderboardProvider =
    FutureProvider.autoDispose.family<TournamentLeaderboard, String>(
  (ref, slug) =>
      ref.watch(tournamentsRepositoryProvider).fetchTournamentLeaderboard(slug),
);
...
class _TournamentDetailScreenState extends ConsumerState<TournamentDetailScreen>
  ...
  Widget build(BuildContext context) {
    final slug = widget.tournamentId;
    final detailAsync = ref.watch(_detailProvider(slug));
    ...
    return Scaffold(
      ...
      body: detailAsync.when(
        ...
        error: (e, _) => _ErrorBody(
          onRetry: () => ref.invalidate(_detailProvider(slug)),
        ),
    ...
    return NestedScrollView(
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _MatchesTab(slug: slug, tournament: tournament),
...
// ── Tournament header (cover + logo + info) ───────────────────────────────────
        ...
        SizedBox(
          ...
          child: Stack(
            ...
            children: [
              ...
              // Full cover gradient: dark top → transparent mid → dark bottom
              Container(
                decoration: BoxDecoration(
              ...
              Transform.translate(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    // Follow button — compact, right-aligned
                    _TournamentFollowButton(tournamentId: t.id),
                  ],
                ),
              ),

              // Badges row (offset up by 20 to close the gap from translate)
              Transform.translate(
...
class _TournamentFollowButton extends StatefulWidget {
  const _TournamentFollowButton({required this.tournamentId});
  final String tournamentId;

  @override
  State<_TournamentFollowButton> createState() =>
      _TournamentFollowButtonState();
}

class _TournamentFollowButtonState extends State<_TournamentFollowButton> {
  ...
  Future<void> _fetchStatus() async {
    try {
      ...
      final res = await dio
          .get(ApiEndpoints.tournamentFollowStatus(widget.tournamentId));
      final following = _extractFollowingValue(res.data);
      if (mounted)
        setState(() {
          _isFollowing = following == true;
          _isLoading = false;
        });
    } catch (_) {
  ...
  bool _extractFollowingValue(dynamic payload) {
    final maps = <Map<dynamic, dynamic>>[];
    if (payload is Map) {
      maps.add(payload);
      final data = payload['data'];
      if (data is Map) maps.add(data);
      final result = payload['result'];
      if (result is Map) maps.add(result);
    }

    for (final map in maps) {
      final raw = map['following'] ?? map['isFollowing'] ?? map['is_following'];
  ...
  Future<void> _toggle() async {
    ...
    try {
      final dio = ApiClient.instance.dio;
      final url = ApiEndpoints.tournamentFollow(widget.tournamentId);
      debugPrint('[TournamentFollow] baseUrl=${dio.options.baseUrl} path=$url');
      if (_isFollowing) {
      ...
      debugPrint('[TournamentFollow] error: $e');
      if (mounted) {
        ...
        child: _isLoading
            ? SizedBox(
                ...
                child: CircularProgressIndicator(
                    strokeWidth: 2, color: _isFollowing ? accent : context.bg),
              )
            : Text(
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text('Highlights',
            style: TextStyle(
                color: context.fg, fontSize: 13, fontWeight: FontWeight.w800)),
...
class _HighlightCard extends StatelessWidget {
  ...
  final TournamentHighlight highlight;
...
(
  ...
  for (var ai = 0; ai < innings.length; ai++) {
    for (var bi = 0; bi < innings.length; bi++) {
      if (ai == bi) continue;
      final score = (ai >= 0 ? aScores[ai] : 0) + (bi >= 0 ? bScores[bi] : 0);
      if (score > bestScore) {
        bestScore = score;
        bestA = ai;
        bestB = bi;
...
int _sideMatchScore({
  ...
  final sideAliases = isA
      ? const {'a', 'teama', 'team1', '1'}
      : const {'b', 'teamb', 'team2', '2'};
  ...
  if (selfNorm.isNotEmpty &&
      (candidate.contains(selfNorm) || selfNorm.contains(candidate))) {
    return 70;
  }
...
class _MatchesTab extends ConsumerStatefulWidget {
  const _MatchesTab({required this.slug, required this.tournament});
  final String slug;
  final TournamentDetail tournament;
...
class _MatchesTabState extends ConsumerState<_MatchesTab> {
  ...
  bool _matchesFilter(TournamentMatch m) {
  ...
  bool _matchesTeamFilter(TournamentMatch m, String? teamName) {
  ...
  @override
  Widget build(BuildContext context) {
    final async = ref.watch(_matchesProvider(widget.slug));
    return async.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, st) {
        if (kDebugMode) debugPrint('matches error: $e');
        return _EmptyTab(
          icon: Icons.sports_cricket_rounded,
      ...
      data: (allMatches) {
        if (kDebugMode) {
          debugPrint('matches count: ${allMatches.length}');
          final sample = allMatches.take(3);
          for (final m in sample) {
            debugPrint(
                '[M] status=${m.status} result=${m.result} innings=${m.innings.length}');
          }
        }
        final teams = <String>{
  ...
  Map<String, List<TournamentMatch>> _grouped(List<TournamentMatch> matches) {
                ...
                children: [
                  ...
                  for (final team in teams)
                    _TeamSelectorTile(
                      ...
                      onTap: () {
                        Navigator.of(context).pop<String?>(team);
                      },
                    ),
                  if (teams.isEmpty)
                    Padding(
...
class _MatchCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Build result string from innings if API didn't provide one
    ...
    if (resolvedResult == null &&
      ...
      if (inn2.totalRuns > inn1.totalRuns) {
        final margin = (10 - inn2.totalWickets);
        resolvedResult = '${m.teamBName} won by $margin wickets';
      } else if (inn1.totalRuns > inn2.totalRuns) {
        final margin = inn1.totalRuns - inn2.totalRuns;
        resolvedResult = '${m.teamAName} won by $margin runs';
      } else {
...
class _SimpleTeamScoreRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final inn = innings;
    final score = inn != null
        ? '${inn.totalRuns}/${inn.totalWickets}'
        : (!isCompleted ? '—' : '-');
    final overs = inn != null ? '(${formatOvers(inn.totalOvers)})' : null;
...
class _StandingsTabState extends ConsumerState<_StandingsTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final async = ref.watch(_standingsProvider(slug));
    return async.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, __) {
        if (kDebugMode) debugPrint('standings error: $e');
                ...
                children: [
                  Expanded(
                    child: GestureDetector(
                      onTap: () => setState(() => _view = _StandingsView.table),
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 160),
...
class _StandingsBracketView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final quarterPairs = <(TournamentStanding, TournamentStanding)>[];
    if (seeds.length >= 8) {
          ...
          children: [
            ...
            if (quarterPairs.length == 4) ...[
              ...
              const _BracketColumn(
                title: 'Final',
                cards: ['Winner SF1 vs Winner SF2'],
              ),
            ],
          ],
        ),
...
class _LeaderboardTabState extends ConsumerState<_LeaderboardTab> {
  ...
  @override
  Widget build(BuildContext context) {
    final lbAsync = ref.watch(_leaderboardProvider(widget.slug));

    return lbAsync.when(
      loading: () => const Center(child: CircularProgressIndicator()),
      error: (e, __) {
        if (kDebugMode) debugPrint('leaderboard error: $e');
        // API failed — show retry message
        return Center(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ...
              TextButton(
                onPressed: () =>
                    ref.invalidate(_leaderboardProvider(widget.slug)),
      ...
      data: (lb) {
        ...
        return _LeaderboardBody(
          ...
          isProvisional: widget.tournamentStatus.toUpperCase() != 'COMPLETED',
        );
      },
    );
  }
...
// ── Real-time leaderboard built from completed match innings ──────────────────
...
class _RealtimeLeaderboard extends StatelessWidget {
  const _RealtimeLeaderboard({required this.matches});
  final List<TournamentMatch> matches;

  @override
  Widget build(BuildContext context) {
    // Aggregate team stats from innings
    final stats = <String, _TeamStat>{};
    for (final m in matches) {
      for (final inn in m.innings) {
        final t = stats.putIfAbsent(
            inn.battingTeam, () => _TeamStat(inn.battingTeam));
        t.played++;
        t.totalRuns += inn.totalRuns;
        if (inn.totalRuns > t.highScore) t.highScore = inn.totalRuns;
        t.totalWickets += inn.totalWickets;
      }
    }
    ...
    return ListView(
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.sync_rounded, size: 13, color: context.accent),
              const SizedBox(width: 6),
              Text(
                  'Live stats from ${matches.length} completed match${matches.length == 1 ? '' : 'es'}',
                  style: TextStyle(
                      color: context.accent,
                      fontSize: 11,
                      fontWeight: FontWeight.w700)),
                        ...
                        children: [
                          ...
                          SizedBox(
                              ...
                              child: Text('${s.played}',
                                  textAlign: TextAlign.center,
                                  style: TextStyle(
                                      color: context.fg, fontSize: 12))),
                          SizedBox(
                              width: 44,
                              child: Text('${s.totalRuns}',
                                  textAlign: TextAlign.center,
                                  style: TextStyle(
                                      color: context.accent,
                                      fontSize: 12,
...
class _LeaderboardBody extends StatefulWidget {
  ...
  final TournamentLeaderboard lb;
  final int section;
...
class _LeaderboardBodyState extends State<_LeaderboardBody> {
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                '4s ${totals.totalFours}  •  6s ${totals.totalSixes}',
                style: TextStyle(
                  color: context.fgSub,
                  fontSize: 10,
                  fontWeight: FontWeight.w600,
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          color:
                              widget.section == i ? context.bg : context.fgSub,
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
                        ),
                      ),
  ...
  List<Widget> _buildOverallIp(BuildContext context, TournamentLeaderboard lb) {
    // Merge all players, deduplicate by player ID, sort by totalIp
    final seen = <String>{};
    final all = [
      ...lb.topBatsmen,
      ...lb.topBowlers,
  ...
  List<Widget> _buildBatsmen(
      BuildContext context, List<LeaderboardPlayer> list) {
    if (list.isEmpty) {
      return [
        _EmptyTab(
            icon: Icons.sports_cricket_rounded,
            message: 'No batting stats yet'),
      ];
    }
    return [
...
class _LeaderboardShareSheetState extends State<_LeaderboardShareSheet> {
  ...
  Future<void> _capture() async {
    ...
    try {
      // Small delay to ensure the card is rendered before capture
      ...
      await Share.shareXFiles(
        ...
        text:
            '🏆 ${widget.tournamentName} — IP Leaderboard\n\nPosted via Swing',
      );
    } catch (e) {
...
class _LeaderboardFlexCard extends StatelessWidget {
  ...
  List<LeaderboardPlayer> get _players {
  ...
  String _topMetric(LeaderboardPlayer p) => switch (section) {
        0 => '${p.totalIp} IP',
        1 => '${p.runs} Runs',
        2 => '${p.wickets} Wkts',
  ...
  String _topSubMetric(LeaderboardPlayer p) => switch (section) {
        0 => '${p.runs}R • ${p.wickets}W • ${p.totalDismissals}D',
        1 => 'SR ${p.strikeRate.toStringAsFixed(1)} • IP ${p.totalIp}',
        2 =>
          'Econ ${p.economy.toStringAsFixed(1)} • Ovs ${formatOvers(p.oversBowled)} • IP ${p.totalIp}',
        3 => 'Ct ${p.catches} • RO ${p.runOuts} • IP ${p.totalIp}',
        _ => 'IP ${p.totalIp}',
      };

  String _listMetric(LeaderboardPlayer p) => switch (section) {
        0 => '${p.totalIp} IP',
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          image: rest[i].player.avatarUrl != null
                              ? DecorationImage(
                                  image:
                                      CachedNetworkImageProvider(rest[i].player.avatarUrl!),
                                  fit: BoxFit.cover,
                                )
                              : null,
                        ),
          ...
          if (isProvisional)
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border(
                    bottom:
                        BorderSide(color: context.warn.withValues(alpha: 0.2))),
              ),
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 5),
                  Text(
                    'Provisional — tournament in progress',
                    style: TextStyle(
                        color: context.warn,
                        fontSize: 10,
                        fontWeight: FontWeight.w600),
              ...
              child: Center(
                child: _sharing
                    ? SizedBox(
                        ...
                        height: 18,
                        child: CircularProgressIndicator(
                            strokeWidth: 2, color: const Color(0xFF0E1613)),
                      )
                    : const Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
            ...
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: p.avatarUrl != null
                      ? DecorationImage(
                          image: CachedNetworkImageProvider(p.avatarUrl!), fit: BoxFit.cover)
                      : null,
                ),
                child: p.avatarUrl == null
                    ? Icon(Icons.person_rounded, color: context.fgSub, size: 28)
                    : null,
              ),
              const SizedBox(width: 14),
              Expanded(
...
class _LeaderboardRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color:
            isTopRank ? context.accent.withValues(alpha: 0.08) : context.cardBg,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isTopRank
          ...
          children: [
            Container(
              ...
              child: team.teamLogoUrl == null
                  ? Icon(Icons.groups_rounded, color: context.accent, size: 16)
                  : null,
            ),
            const SizedBox(width: 8),
            Expanded(
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color:
                              team.isConfirmed ? context.success : context.warn,
                          shape: BoxShape.circle,
                        ),
                      ),
                      Text(
            ...
            if (team.teamId != null)
              Icon(Icons.chevron_right_rounded, color: context.fgSub, size: 16),
          ],
        ),
      ),
    );
...
class _MatchResultFlexCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (resolvedResult == null &&
      ...
      if (inn2.totalRuns > inn1.totalRuns) {
        resolvedResult =
            '${m.teamBName} won by ${10 - inn2.totalWickets} wickets';
      } else if (inn1.totalRuns > inn2.totalRuns) {
        resolvedResult =
            '${m.teamAName} won by ${inn1.totalRuns - inn2.totalRuns} runs';
      } else {
        resolvedResult = 'Match tied';
      }
    }
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(children: [
              Expanded(
                  child: Divider(color: context.stroke.withValues(alpha: 0.5))),
              Padding(
  ...
  Widget build(BuildContext _) {
    ...
    return Row(
      children: [
        ...
        if (inn != null)
          Text(
            '${inn.totalRuns}/${inn.totalWickets} (${formatOvers(inn.totalOvers)})',
            style: TextStyle(
                color: context.fg, fontSize: 14, fontWeight: FontWeight.w800),
          ),
      ],
          ...
          if (isProvisional) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(8),
                border: Border.all(color: context.warn.withValues(alpha: 0.35)),
              ),
              child: Text(
                'PROVISIONAL',
                style: TextStyle(
...
class _AwardRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Icon(icon, size: 13, color: context.accent),
        const SizedBox(width: 6),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
