...
import '../../create_tournament/presentation/create_tournament_screen.dart';
...
class _HostTournamentViewerScreenState
  ...
  Future<void> _showEditSheet(
      BuildContext context, TournamentDetailModel tournament) async {
    // Fetch the raw tournament map first so the shared create-tournament
    // stepper hydrates every field (customOvers, category, ageGroup,
    // seriesMatchCount, isPublic, etc. aren't on TournamentDetailModel).
    Map<String, dynamic>? initial;
    try {
      initial = await ref
          .read(hostTournamentRepositoryProvider)
          .getTournament(tournament.id);
    } catch (_) {
      // If the fetch fails, fall through with the bits we already know
      // from the detail model — the user can still edit the visible
      // fields; missing ones default in the stepper.
    }
    initial ??= <String, dynamic>{
      ...
      if (tournament.city != null) 'city': tournament.city,
      if (tournament.venueName != null) 'venueName': tournament.venueName,
      if (tournament.logoUrl != null) 'logoUrl': tournament.logoUrl,
      if (tournament.coverUrl != null) 'coverUrl': tournament.coverUrl,
      if (tournament.description != null) 'description': tournament.description,
      if (tournament.entryFee != null) 'entryFee': tournament.entryFee,
      if (tournament.prizePool != null) 'prizePool': tournament.prizePool,
      if (tournament.organiserName != null)
        'organiserName': tournament.organiserName,
      if (tournament.organiserPhone != null)
    ...
    if (!mounted) return;
    ref.invalidate(hostTournamentDetailProvider(widget.slug));
  }
