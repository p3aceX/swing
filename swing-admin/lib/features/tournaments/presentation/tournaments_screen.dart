...
enum TournamentStatusFilter {
  all('All', null, '/tournaments'),
  live('Live', 'LIVE', '/tournaments/live'),
  scheduled('Scheduled', 'UPCOMING', '/tournaments/scheduled'),
  complete('Complete', 'COMPLETED', '/tournaments/complete');

  const TournamentStatusFilter(this.label, this.apiStatus, this.path);
...
class TournamentsScreen extends ConsumerStatefulWidget {
  const TournamentsScreen({super.key, this.status = TournamentStatusFilter.all});

  final TournamentStatusFilter status;
...
class _TournamentsScreenState extends ConsumerState<TournamentsScreen> {
  ...
  void _showTrendSheet() {
    showModalBottomSheet<void>(
      ...
      builder: (_) => const _TournamentsTrendSheet(),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final query = TournamentsQuery(
      status: widget.status.apiStatus,
      search: _search.isEmpty ? null : _search,
    );
    final asyncPage = ref.watch(tournamentsListProvider(query));
    final summary = ref.watch(tournamentsSummaryProvider);
      ...
      body: SafeArea(
        ...
        child: RefreshIndicator(
          onRefresh: () async {
            ...
            ref.invalidate(tournamentsListProvider(query));
            await ref.read(tournamentsListProvider(query).future);
          },
          child: ListView(
            ...
            children: [
              ...
              _StatusTabs(
                ...
                tabs: TournamentStatusFilter.values
              ...
              _SearchField(
                hintText: 'Search tournaments, city, or venue',
                onChanged: (value) => setState(() => _search = value.trim()),
              ),
              const SizedBox(height: 12),
              asyncPage.when(
                data: (page) => _TournamentsList(page: page),
                loading: () => const _LoadingCard(),
                error: (error, _) => _ErrorCard(
                  message: error.toString(),
                  onRetry: () => ref.invalidate(tournamentsListProvider(query)),
                ),
              ),
            ],
          ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Review tournament growth and execution.',
                style: TextStyle(
                  fontSize: 13,
...
class _SummaryStrip extends StatelessWidget {
  ...
  final AsyncValue<TournamentOpsSummary> async;
...
class _TournamentsList extends StatelessWidget {
  const _TournamentsList({required this.page});

  final AdminTournamentsPage page;

  @override
  Widget build(BuildContext context) {
    if (page.tournaments.isEmpty) {
      return const _EmptyCard(message: 'No tournaments found for this state.');
    }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          for (var index = 0; index < page.tournaments.length; index++) ...[
            _TournamentRow(tournament: page.tournaments[index]),
            if (index < page.tournaments.length - 1)
...
class _TournamentRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final dateRange = _buildDateRange(
      ...
      fallback: tournament.createdAt,
    );
    final venue = tournament.venueName.trim().isEmpty
        ? 'Venue pending'
          ...
          children: [
            Row(
              ...
              children: [
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        city.isEmpty ? '$venue • $dateRange' : '$city • $dateRange',
                        style: const TextStyle(
                          fontSize: 13,
                          color: AppColors.textSecondary,
                        ),
                ...
                _StatusBadge(
                  label: _formatStatus(tournament.status),
                  tone: _toneForTournament(tournament.status),
                ),
              ],
            ),
            const SizedBox(height: 10),
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _MetaPill(icon: Icons.place_outlined, label: venue),
                if (tournament.format.trim().isNotEmpty)
                  _MetaPill(
                      icon: Icons.grid_view_rounded,
                      label: _formatStatus(tournament.format)),
                _MetaPill(
                  icon: Icons.groups_outlined,
                  label:
                      '${tournament.teamCount}/${tournament.maxTeams == 0 ? 'NA' : tournament.maxTeams} teams',
                ),
                if (tournament.seriesMatchCount != null)
                  _MetaPill(
                    icon: Icons.summarize_outlined,
                    label: '${tournament.seriesMatchCount} matches',
                  ),
              ],
            ),
          ],
...
class _TournamentsTrendSheet extends ConsumerStatefulWidget {
  const _TournamentsTrendSheet();

  @override
  ConsumerState<_TournamentsTrendSheet> createState() =>
      _TournamentsTrendSheetState();
}

class _TournamentsTrendSheetState extends ConsumerState<_TournamentsTrendSheet> {
  _TrendGranularity _granularity = _TrendGranularity.day;

  @override
  Widget build(BuildContext context) {
    final async = ref.watch(tournamentsTrendItemsProvider);
    return SafeArea(
      child: Padding(
        padding: const EdgeInsets.fromLTRB(16, 4, 16, 20),
        child: async.when(
          data: (items) {
            final points = _buildTournamentTrendPoints(items, _granularity);
            ...
            return Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  '$total tournaments in the selected trend window',
                  style: const TextStyle(
                    fontSize: 13,
                    color: AppColors.textSecondary,
                  ),
...
List<_TrendPoint> _buildTournamentTrendPoints(
  List<AdminTournament> items,
  ...
  for (final item in items) {
    final source = item.startDate ?? item.createdAt;
...
Color _toneForTournament(String status) {
  switch (status) {
    case 'LIVE':
...
String _buildDateRange({
  ...
  if (start == null && end == null) {
    if (fallback == null) return 'Dates pending';
    return DateFormat('dd MMM yyyy').format(fallback.toLocal());
  }
  final startLabel =
      start == null ? 'TBD' : DateFormat('dd MMM').format(start.toLocal());
  final endLabel =
      end == null ? 'TBD' : DateFormat('dd MMM').format(end.toLocal());
