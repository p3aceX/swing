...
export class AdminService {
  ...
  // ── Phase 5: Backfill MatchRole for all existing matches ──────────────────

  async backfillMatchRoles(): Promise<{ processed: number; skipped: number; errors: number }> {
    const BATCH = 100
    ...
    for (;;) {
      ...
      for (const match of matches) {
        try {
          // Check if this match already has any MatchRole rows
          ...
          // For non-tournament matches, captains → MANAGER
          if (!match.tournamentId) {
          ...
          await prisma.$transaction(async (tx) => {
            ...
            // Set activeScorerId from scorerId if not already set
          ...
          console.error('[backfillMatchRoles] error matchId=%s', match.id, err)
          errors++
        }
      }
    }
