...
export async function playerRoutes(app: FastifyInstance) {
  ...
  app.post('/teams', auth, async (request, reply) => {
    ...
    const body = z.object({
      ...
      ageGroup: z.enum(['U14', 'U16', 'U19', 'U23', 'SENIOR']),
      ...
      homeGroundName: z.string().max(120).optional(),
      foundedYear: z.number().int().min(1800).max(2100).optional(),
      format: z.enum(['T20', 'ODI', 'TEST', 'ALL']).optional(),
  ...
  app.patch('/teams/:id', auth, async (request, reply) => {
    ...
    const body = z.object({
      ...
      ageGroup: z.enum(['U14', 'U16', 'U19', 'U23', 'SENIOR']).optional(),
      logoUrl: z.string().url().optional(),
      motto: z.string().max(120).optional(),
      homeGroundName: z.string().max(120).optional(),
  ...
  app.get('/search', auth, async (request, reply) => {
    ...
    const q = z.object({
      ...
      teamType: z.enum(['SCHOOL', 'CLUB_ACADEMY', 'CORPORATE', 'GULLY', 'ASSOCIATION']).optional(),
  ...
  app.patch('/tournaments/:id', auth, async (request, reply) => {
    ...
    const body = z.object({
      ...
      customOvers: z.number().int().min(1).max(50).nullable().optional(),
      category: z.enum(['SCHOOL', 'CLUB_ACADEMY', 'CORPORATE', 'GULLY', 'ASSOCIATION']).optional(),
      ageGroup: z.enum(['U14', 'U16', 'U19', 'U23', 'SENIOR']).optional(),
