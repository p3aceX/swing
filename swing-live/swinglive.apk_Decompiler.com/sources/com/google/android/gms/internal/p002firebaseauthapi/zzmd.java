...
public final class zzmd {
    ...
    private static zzmg zza(zzmg zzmgVar, zzmg zzmgVar2, BigInteger bigInteger, BigInteger bigInteger2) {
        ...
        BigInteger bigIntegerMod3 = zzmgVar.zzb.multiply(bigIntegerMod2).mod(bigInteger2);
        BigInteger bigIntegerMod4 = zzmgVar2.zzb.multiply(bigIntegerMod).mod(bigInteger2);
        BigInteger bigIntegerMod5 = zzmgVar.zzc.multiply(zzmgVar2.zzd).mod(bigInteger2).multiply(bigIntegerMod2).mod(bigInteger2);
        BigInteger bigIntegerMod6 = zzmgVar2.zzc.multiply(zzmgVar.zzd).mod(bigInteger2).multiply(bigIntegerMod).mod(bigInteger2);
        if (bigIntegerMod3.equals(bigIntegerMod4)) {
            return !bigIntegerMod5.equals(bigIntegerMod6) ? zzmg.zza : zza(zzmgVar, bigInteger, bigInteger2);
        }
        ...
        BigInteger bigIntegerAdd = zzmgVar.zzd.add(zzmgVar2.zzd);
        return new zzmg(bigIntegerMod12, bigIntegerMod13, bigIntegerAdd.multiply(bigIntegerAdd).mod(bigInteger2).subtract(bigIntegerMod).subtract(bigIntegerMod2).multiply(bigIntegerMod7).mod(bigInteger2));
    }
    ...
    private static zzmg zza(zzmg zzmgVar, BigInteger bigInteger, BigInteger bigInteger2) {
        ...
        BigInteger bigIntegerAdd = zzmgVar.zzb.add(bigIntegerMod2);
        ...
        BigInteger bigIntegerAdd3 = zzmgVar.zzc.add(zzmgVar.zzd);
        return new zzmg(bigIntegerMod5, bigIntegerMod6, bigIntegerAdd3.multiply(bigIntegerAdd3).mod(bigInteger2).subtract(bigIntegerMod2).subtract(bigIntegerMod4).mod(bigInteger2));
    }
    ...
    private static zzmg zza(ECPoint eCPoint, BigInteger bigInteger) {
        ...
        BigInteger bigIntegerMod = new BigInteger(1, zzov.zza((bigInteger.bitLength() + 8) / 8)).mod(bigInteger);
        BigInteger bigIntegerMod2 = bigIntegerMod.multiply(bigIntegerMod).mod(bigInteger);
        return new zzmg(eCPoint.getAffineX().multiply(bigIntegerMod2).mod(bigInteger), eCPoint.getAffineY().multiply(bigIntegerMod2.multiply(bigIntegerMod).mod(bigInteger)).mod(bigInteger), bigIntegerMod);
    }
    ...
    public static ECPoint zza(BigInteger bigInteger, ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        ...
        if (bigInteger.signum() == 1) {
            if (bigInteger.compareTo(eCParameterSpec.getOrder()) < 0) {
                ...
                zzmg zzmgVarZza = zza(ECPoint.POINT_INFINITY, bigIntegerZza);
                zzmg zzmgVarZza2 = zza(generator, bigIntegerZza);
                for (int iBitLength = bigInteger.bitLength(); iBitLength >= 0; iBitLength--) {
                    if (bigInteger.testBit(iBitLength)) {
                        zzmgVarZza = zza(zzmgVarZza, zzmgVarZza2, a5, bigIntegerZza);
                        zzmgVarZza2 = zza(zzmgVarZza2, a5, bigIntegerZza);
                    } else {
                        zzmgVarZza2 = zza(zzmgVarZza, zzmgVarZza2, a5, bigIntegerZza);
                        zzmgVarZza = zza(zzmgVarZza, a5, bigIntegerZza);
                    }
                }
                if (zzmgVarZza.zza()) {
                    eCPoint = ECPoint.POINT_INFINITY;
                } else {
                    BigInteger bigIntegerModInverse = zzmgVarZza.zzd.modInverse(bigIntegerZza);
                    BigInteger bigIntegerMod = bigIntegerModInverse.multiply(bigIntegerModInverse).mod(bigIntegerZza);
                    eCPoint = new ECPoint(zzmgVarZza.zzb.multiply(bigIntegerMod).mod(bigIntegerZza), zzmgVarZza.zzc.multiply(bigIntegerMod).mod(bigIntegerZza).multiply(bigIntegerModInverse).mod(bigIntegerZza));
                }
                zza(eCPoint, curve);
                return eCPoint;
            }
