...
public final class Tasks {
    ...
    public static <TResult> TResult await(Task<TResult> task) throws InterruptedException {
        ...
        F.h(task, "Task must not be null");
        if (task.isComplete()) {
    ...
    public static <T> Task<T> withTimeout(Task<T> task, long j4, TimeUnit timeUnit) {
        F.h(task, "Task must not be null");
        F.a("Timeout must be positive", j4 > 0);
    ...
    public static <TResult> TResult await(Task<TResult> task, long j4, TimeUnit timeUnit) throws TimeoutException {
        F.f("Must not be called on the main application thread");
        F.h(task, "Task must not be null");
        F.h(timeUnit, "TimeUnit must not be null");
        if (task.isComplete()) {
