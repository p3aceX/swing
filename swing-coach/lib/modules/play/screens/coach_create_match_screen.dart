...
class CoachCreateMatchScreen extends ConsumerStatefulWidget {
  const CoachCreateMatchScreen({
    super.key,
  ...
  @override
  ConsumerState<CoachCreateMatchScreen> createState() =>
      _CoachCreateMatchScreenState();
}

class _CoachCreateMatchScreenState
    extends ConsumerState<CoachCreateMatchScreen> {
  ...
  void _onTossCompleted(BuildContext context, String matchId) {
    context.push('/play/score/${Uri.encodeComponent(matchId)}');
  }
        ...
        return Scaffold(
          ...
          body: Center(
            child: Padding(
              ...
              child: Text(_resumeError ?? 'Could not load this match.'),
            ),
          ),
        );
      }
